...
class ManipulationImage:
    ...
    def save_image(self, data, img_name):
        ...
        title_subplot = file_name+'_th.png'

        # Path and name of the image to save, with data.
        #misc.imwrite('images/outputs/' + title_subplot, data)


        nueva = img.fromarray(np.uint8(data))
        nueva.save('images/outputs/' + title_subplot)
...
# ---------------------------------------------------------------------------------------------------------------------- #
        ...
        def print_images(self, images, titles, se_name, se_params):
            ...
            for i in range(3):
                ...
                plt.xlabel('Pixels Val: ' + str(int(plt.hist(images[i * 4].ravel(), 256)[1].min())) + ' to ' + str(
                    int(plt.hist(images[i * 4].ravel(), 256)[1].max()))), \
                plt.ylabel('Freq. Val: ' + str(int(plt.hist(images[i * 4].ravel(), 256)[0].min())) + ' to ' + str(
                    int(plt.hist(images[i * 4].ravel(), 256)[0].max()))),
            ...
            plt.annotate('Structuring Element: ' + se_name + ', ' + str(se_params), xy=(1, 1), xytext=(-8, 355),
                         fontsize=11,
